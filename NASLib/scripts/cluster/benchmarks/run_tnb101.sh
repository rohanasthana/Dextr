...
if [ -z "$predictor" ];
then
    predictors=(dextr) #fisher  grad_norm grasp jacov snip synflow epe_nas flops params plain l2_norm nwot)
    memory=(64G)  #64G     32G       64G   32G   32G  32G     32G     5G    5G     32G   32G     32G)
else
...
do
    ...
    do
        ...
        do
            ...
            echo $pred $dataset
            bash ./scripts/cluster/benchmarks/run.sh $searchspace $dataset $pred $start_seed $experiment $n_models $start --bosch
